...
class Verdict(Enum):
    ...
    SUSPECT_OK = "suspect"         # ambiguous (abck unresolved / soft) → NON-terminal
...
TERMINAL_NONSUCCESS = frozenset({
    Verdict.AUTH_REQUIRED, Verdict.LOGIN_WALL, Verdict.PAYWALL,
    Verdict.NOT_FOUND, Verdict.UNSAFE_URL,
})
...
def _wall_verdict(lowered: str) -> Optional[tuple[Verdict, str]]:
    compact = re.sub(r"\s+", " ", lowered)
    if re.search(r"<input[^>]+type=[\"']?password", lowered):
        return Verdict.LOGIN_WALL, "password_field"
    if re.search(r"<form[^>]+(login|signin|sign-in|auth)", lowered):
        return Verdict.LOGIN_WALL, "login_form"
    login_markers = (
        "sign in to continue",
        "log in to continue",
        "login required",
    ...
    if any(marker in compact for marker in login_markers):
        return Verdict.LOGIN_WALL, "login_marker"
    paywall_markers = (
    ...
    if any(marker in compact for marker in paywall_markers):
        return Verdict.PAYWALL, "paywall_marker"
    consent_markers = (
        "accept all cookies",
        "cookie consent",
    ...
    if any(marker in compact for marker in consent_markers):
        return Verdict.CONSENT_WALL, "consent_marker"
    captcha_markers = (
        "g-recaptcha",
        "h-captcha",
        "cf-turnstile",
...
def validate(
    ...
    *,
    ...
    if status == 0:
        ...
        return r
    if status == 204 and success_selectors:
        r.verdict = Verdict.INVALID_CONTENT
        r.reasons.append("status=204:no_content_for_selector")
        return r
