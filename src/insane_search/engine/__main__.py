#!/usr/bin/env python3
"""CLI entrypoint for the Namba Search engine.

Usage:
    python3 -m insane_search.engine URL [--selector CSS] [--device auto|desktop|mobile]
                                  [--timeout N] [--max-attempts N] [--json] [--trace]

Examples:
    python3 -m insane_search.engine "https://example.com/" --selector "h1"
    python3 -m insane_search.engine "https://example.com/" --json
    python3 -m insane_search.engine "https://example.com/" --device mobile --trace
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python3 -m insane_search.engine",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "R7 API-first" in (result.summary or ""):
        print("R7 triggered: consider public API-first route instead of HTML grid.", file=sys.stderr)
    ...
    if not result.ok and (result.untried_routes or result.must_invoke_playwright_mcp):
        print(
            "NOT EXHAUSTED: do not declare failure yet. "
        ...
        if result.must_invoke_playwright_mcp:
            print("   internal browser fallback is still required.", file=sys.stderr)
