"""Single entrypoint: Namba Search generic fetch chain.
...
def _fetch_core(
    ...
    *,
    ...
    # If a terminal-nonsuccess (404/auth/paywall) stopped us, browser won't help.
        ...
        try:
            ...
            for fb_name in fb_order:
                ...
                pw_attempt, pw_content = run_playwright_fallback(
                    ...
                    force_executor=fb_name, timeout=max(2, min(timeout or 30, 30)),
                )
            ...
            trace.append(Attempt(
                ...
                url_transform="original", impersonate=None, referer="",
                verdict=Verdict.BROWSER_UNAVAILABLE.value, error="executor module not available"))
...
def _untried_routes(stop_reason, grid_exhausted) -> tuple[list[str], bool]:
    ...
    Returns (untried_routes, must_invoke_playwright_mcp). The second value is
    kept for backward-compatible result shape and is always false in the Codex
    plugin path because the service owns browser fallback internally.
    """
    routes: list[str] = []
    # 429 is TRANSIENT, not a wall — exclude it from terminal so the gate still
    # surfaces backoff/retry instead of telling the agent to give up (the exact
    ...
    if rate_limited:
        routes.append("rate-limited (429): transient; back off before retrying public routes. Do not hammer the grid.")
    ...
    routes.append("internal_browser_fallback: retry with mode='browser_allowed' when a local isolated browser is available")
...
R7_HINT = (
    "R7 API-first recommended: the HTML path appears gated by WAF. "
    "Use only same-origin public API candidates discovered by the internal "
    "browser adapter, then revalidate each candidate URL through NetworkPolicy."
)
