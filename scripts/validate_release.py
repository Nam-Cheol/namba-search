...
def main() -> int:
    ...
    if "namba-search" not in mcp:
        ...
        for arg in server.get("args", []):
            if isinstance(arg, str) and arg.startswith("./") and not (ROOT / arg).exists():
    ...
    elif marketplace["plugins"][0].get("source", {}).get("path") != "./":
        errors.append("repository marketplace must point at plugin root './'")
    skill = ROOT / "skills" / "namba-search" / "SKILL.md"
    agent = ROOT / "skills" / "namba-search" / "agents" / "openai.yaml"
    if not skill.exists():
